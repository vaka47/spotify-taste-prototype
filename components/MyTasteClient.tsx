      ...
      {state === "connected" && user ? (
        <>
          ...
          {activeTab === "settings" ? <section id="owner-panel-settings" className="spxOwnerSettings spxOwnerSettingsPanel" role="tabpanel" aria-labelledby="owner-tab-settings">
            ...
            <div className="spxOwnerSettingsLinks"><Link href="/hub"><Icon name="hub" size={16} />{locale === "ru" ? "Кабинет автора" : "Tastemaker Hub"}</Link><Link href="/privacy"><Icon name="privacy" size={16} />{t("nav.privacy")}</Link><button type="button" onClick={logout}><Icon name="logout" size={16} />{locale === "ru" ? "Выйти" : "Log out"}</button></div>
          </section> : null}
